...
flowchart LR
    ...
    %% ========================================
    subgraph Tiers["Three-Tier Promotion Flow"]
        direction LR
        %% ========================================
        %% TIER 1: INNOVATION LAB
        %% ========================================
        subgraph Tier1["🌍 TIER 1: INNOVATION LAB<br/>(Public PKI) - 192.168.0.0/16"]
            direction TB
            
            subgraph T1Network["🌐 NETWORK PLATFORM ENVER"]
                direction LR
                T1TGW["🔗 Transit Gateway<br/>192.168.0.1"]
                T1IPAM["📊 IPAM Pool<br/>192.168.0.0/16"]
            end
            
            subgraph T1Platform["🏗️ PLATFORM SERVICE VPCs"]
                direction LR
                T1DBVPC["🗄️ DB Platform VPC"]
                T1EKSVPC["☸️ EKS Platform VPC"]
            end
            
            subgraph T1Apps["📱 APPLICATION ENVER VPCs"]
                direction TB
                T1App1VPC["🚀 App-A Main VPC<br/>React + Node.js"]
                T1App2VPC["🚀 App-B Main VPC<br/>Python ML"]
                T1App3VPC["🚀 App-C Main VPC<br/>Java Spring Boot"]
                T1App4VPC["🚀 App-D Main VPC<br/>Go Microservices"]
            end
        end
        ...
        %% TIER 2: QUARANTINE
        %% ========================================
        subgraph Tier2["🔍 TIER 2: QUARANTINE<br/>(Hybrid PKI) - 172.16.0.0/12"]
            direction TB
            
            subgraph T2Network["🌐 NETWORK PLATFORM ENVER"]
                direction LR
                T2TGW["🔗 Transit Gateway<br/>172.16.0.1"]
                T2IPAM["📊 IPAM Pool<br/>172.16.0.0/12"]
            end
            
            subgraph T2Platform["🏗️ PLATFORM SERVICE VPCs"]
                direction LR
                T2DBVPC["🗄️ DB Platform VPC<br/>Security Tools"]
                T2EKSVPC["☸️ EKS Platform VPC<br/>Scan Cluster"]
            end
            
            subgraph T2Apps["📱 APPLICATION ENVER VPCs"]
                direction TB
                T2App1VPC["🛡️ App-A VPC<br/>Security Scanning"]
                T2App2VPC["🛡️ App-B VPC<br/>License Compliance"]
                T2App3VPC["🛡️ App-C VPC<br/>Container Scanning"]
                T2App4VPC["🛡️ App-D VPC<br/>Binary Analysis"]
            end
        end
        ...
        %% TIER 3: INTERNAL POC
        %% ========================================
        subgraph Tier3["🔒 TIER 3: INTERNAL POC<br/>(Private PKI) - 10.0.0.0/8"]
            direction TB
            
            subgraph T3Network["🌐 NETWORK PLATFORM ENVER"]
                direction TB
                subgraph T3NetworkRow1["Network Infrastructure"]
                    direction LR
                    T3TGW["🔗 Transit Gateway<br/>10.0.0.1"]
                    T3IPAM["📊 IPAM Pool<br/>10.0.0.0/8"]
                end
                T3VPCEndpoints["🔌 VPC Endpoints<br/>Private AWS Services"]
            end
            
            subgraph T3Platform["🏗️ PLATFORM SERVICE VPCs"]
                direction LR
                T3DBVPC["🗄️ DB Platform VPC<br/>Private PKI DBs"]
                T3EKSVPC["☸️ EKS Platform VPC<br/>Internal POC Cluster"]
            end
            
            subgraph T3Apps["📱 APPLICATION ENVER VPCs"]
                direction TB
                T3App1VPC["⚙️ App-A VPC<br/>Private PKI App"]
                T3App2VPC["⚙️ App-B VPC<br/>Zero Internet App"]
                T3App3VPC["⚙️ App-C VPC<br/>Internal POC App"]
                T3App4VPC["⚙️ App-D VPC<br/>Secure Internal App"]
            end
        end
    end

    %% ========================================
    %% External Systems & Dynamic Capability
    %% ========================================
    subgraph ExternalSystems["External Systems & Capabilities"]
        direction TB
        Internet["🌐 Public Internet<br/>Repos & APIs"]
        InternalRepo["📦 Internal Artifactory<br/>Approved Packages"]
        subgraph DynamicCapability["🌿 DYNAMIC ENVER CREATION"]
            direction TB
            T1App1Branch["🌿 App-A feature/auth VPC<br/>Auto-created on Git branch"]
            BranchFlow["Git Branch → Auto-Creates Enver"]
        end
    end
    ...
    T1TGW --- T1Platform
    T1Platform ==> T1Apps
    T1TGW -.->|"🌐 Internet via NAT"| Internet
    T1App1VPC -.->|"odmd: create@main"| T1App1Branch

    %% Tier 2 Connections
    T2TGW --- T2Platform
    T2Platform ==> T2Apps
    T2TGW -.->|"📦 Internal Repos Only"| InternalRepo

    %% Tier 3 Connections
    T3TGW --- T3Platform
    T3Platform ==> T3Apps
    T3TGW -.->|"📦 Internal Repos Only"| InternalRepo
    T3TGW --- T3VPCEndpoints

    %% Promotion Flow (Code)
    Tier1 -- "Git Fork App Code" --> Tier2
    Tier2 -- "Security Approval" --> Tier3

    %% Promotion Flow (Platform Infrastructure)
    T1Network -. "Git Fork" .-> T2Network
    T2Network -. "Git Fork" .-> T3Network
    T1Platform -. "Git Fork" .-> T2Platform
    T2Platform -. "Git Fork" .-> T3Platform
    
    %% ========================================
    %% STYLING
    %% ========================================
    %% Tier styling
    style Tier1 fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,stroke-dasharray: 5 5
    style Tier2 fill:#fff3e0,stroke:#ff8f00,stroke-width:3px,stroke-dasharray: 5 5
    style Tier3 fill:#f3e5f5,stroke:#7b1fa2,stroke-width:3px,stroke-dasharray: 5 5
    
    %% Network Platform styling
    style T1Network fill:#c8e6c9,stroke:#388e3c,stroke-width:2px
    style T2Network fill:#ffecb3,stroke:#f57c00,stroke-width:2px
    style T3Network fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px
    ...
    style T1Apps fill:#e3f2fd,stroke:#039be5,stroke-width:1px
    style T2Apps fill:#fff8e1,stroke:#ffa000,stroke-width:1px
    style T3Apps fill:#fce4ec,stroke:#9c27b0,stroke-width:1px
    
    %% Dynamic capability styling
    style DynamicCapability fill:#e8f5e9,stroke:#4caf50,stroke-width:2px,stroke-dasharray: 5 5
    style T1App1Branch fill:#dcedc8,stroke:#66bb6a,stroke-width:2px
    
    %% External Systems Styling
    style Internet fill:#f5f5f5,stroke:#616161,stroke-width:2px
    style InternalRepo fill:#f5f5f5,stroke:#616161,stroke-width:2px
</div>
</div>
