...
The combination of K8s's inherent complexity, its DNA limitations, and the realities of cloud integration creates a significant burden, particularly felt by software engineers.
  * **Focus Dilution & The DDD Conflict:** For engineers practicing Domain-Driven Design (DDD), the primary goal is mastering domain complexity. K8s complexity – learning the objects, writing verbose YAML, debugging networking, managing cloud annotations, dealing with Helm intricacies – directly competes for cognitive load, pulling focus away from building business value. The infrastructure fails to fade into the background.
...
However, blinded by the Gell-Mann Amnesia effect, the industry often overlooks its significant flaws and limitations. As we've seen:
  ...
  * The resulting complexity imposes a heavy burden on developers, often distracting from core business logic and making simpler PaaS alternatives increasingly attractive.
